...
function io_server(robot) {
  ...
  log.info(`Starting socket.io server on IOPORT: ${process.env.IOPORT}`)
  var server
  ...
  global.ioPromise = new Promise((resolve, reject) => {
    global.io.sockets.on('connection', (socket) => {
      ...
      socket.on('join', (id) => {
        socket.join(id)
        log.debug(id, socket.id, 'joined')
        if (--count == 0) {
          log.info(`All ${CLIENT_COUNT} clients have joined`)
