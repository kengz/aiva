...
var polyglotCommands = _.pickBy({
    ruby: [path.join(__dirname, 'client.rb')],
    python3: [path.join(__dirname, 'client.py')]
  },
  (args, cmd) => {
    return global.config.get("ACTIVATE_IO_CLIENTS").get(cmd)
  })

const CLIENT_COUNT = 1 + _.size(polyglotCommands)
...
function io_client(robot) {
  global.log.info("Starting polyglot io_clients")
  ...
  _.each(polyglotCommands, (args, cmd) => {
    // spawn then add listeners, add to the list of child processes
    var cp = spawn(cmd, args, { stdio: [process.stdin, process.stdout, 'pipe'] })
    children.push(cp)

    /* istanbul ignore next */
    cp.stderr.on('data', (data) => {
      global.log.error('err:', data.toString('utf8'))
