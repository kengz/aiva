...
if (process.env.ADAPTERS) {
  clientCount += process.env.ADAPTERS.split(",").length
};
...
function io_server(robot) {
  ...
  log.info('Starting io_server.js: socket.io on Express server of robot.')
  var server;
  if (robot) {
    server = robot.server;
  } else {
...
function io_start(robot) {
  process.env.IOPORT = process.env.IOPORT || 6363
  io_server(robot)
  return global.ioPromise
}
